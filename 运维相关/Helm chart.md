...
- 可以看到，在 `mug: true` 前后各多了一个空行，因为在模板渲染的过程中，删除了`{{`和`}}`中的内容，但保留了其余的空白。
- **`-`消除模板语句占用的空格**。
- 在下列示例中，星号(\*)表示被删除的空格：
...
- 渲染出来的结果会是：`food: "PIZZA"mug:true`。 
- Helm还提供了`indent`函数增加空格来进行缩进：
...
- 结合`Files`内置变量，可以在模板中嵌入文件内容;
- 其中最常见的用法就是将**应用的配置文件作为 `ConfigMap` 或者是 `Secret` 随应用一起发布**。
- 内置变量中`Files.AsConfig`和`Files.AsSecrets`就是用来把配置文件转换成`ConfigMap`和`Secret`的。
