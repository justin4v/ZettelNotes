...
# Interface
因为对象具有责任而且自己负责自己，所以必须有办法告诉对象要做什么。对象含有说明自己状态的数据，还有实现必要功能的方法。对象的很多方法都将标识为可被其他对象调用。这些方法的集合就称为对象的公开接口（public interface）。
...
# Class
要求多个 Student 对象都有自己的一组方法，告诉它能做什么和怎样做 ，显然效率很低
让所有学生与一组方法关联起来，每个学生都可以根据自己的需要使用或修改这些方法。我希望定义一个“一般学生”来包含这些公共方法的定义。然后，可以有各种各样特殊的学生，每个特殊学生都必须掌握自己的私有信息。
