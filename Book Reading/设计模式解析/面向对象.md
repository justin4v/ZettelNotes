...
## Martin Flower视角框架
从[[开发过程中的三个视角|三个视角]]：
- At the **conceptual(概念)** level, an object is **a set of responsibilities**.
- At the **specification(规约)** level, an object is **a set of methods (behaviors)** that can be invoked by other objects or by itself. 
