...
- 任意比较复杂的数据结构，都可以以多种不同方式用基本数据结构表示。
- 表示（representation）方式的选择会对操作它的 procedure 产生影响。
- 对于大型程序而言，表示方式的改变代价极其昂贵，需要在设计时将依赖于 representation 部分限制在很少的几个 program modules。

如将有理数约化到最简形式，可以不在构造的时候做，而是在每次访问有理数中时去做，将导致另一套不同的构造函数和选择函数：
```scheme
(define (make-rat n d)
  (cons n d))
(define (numer x)
