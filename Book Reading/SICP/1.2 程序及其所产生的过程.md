...
### 概率方法
- 从特征上看，费马检查与前面的算法都不一样。
- 前面算法都保证计算的结果一定正确，而费马检查得到的结果只有概率上的正确性；
- 如果数 n 不能通过费马检查，可以确信它一定不是素数。
- 而 n 通过了费马只能作为它是素数的一个很强的证据，但却不是对 n 为素数的保证。
- 存在着一些能骗过费马检查的整数：某些数n不是素数但却具有这样的性质，对任意整数a＜n，都有 $a^n$ 与 a 模 n 同余。这种数极其罕见，因此费马检查在实践中还是很可靠的。
- 可以证明，对任意的数n，费马检查对整数 a＜n 大部分都不成立，除非n是素数。这样，如果n对某个随机选出的a能通过检查，n是素数的机会就大于一半