...
- a *receiving object* delegates operations to its *delegate*（委托人）. This is analogous（like） to subclasses deferring requests to parent classes. 
- But with inheritance, an inherited operation can always refer to the receiving object through the **this** member variable in C++ and self in Smalltalk. Toachieve the same effect with delegation, the receiver passesitself to the delegate to let the delegated operation refer to the receiver