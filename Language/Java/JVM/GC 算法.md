...
- 当一个对象到 GC Roots 没有任何引用链相连时，则证明此对象是不可用的，**不可达对象**。
- 即给定一个集合的引用作为根出发，通过引用关系遍历*对象图*，**能被遍历到的（可到达的）对象就被判定为存活，没有被遍历到的就自然被判定为死亡**。
...
1. 优点
	1. 没有内存碎片
2. 缺点
	1. 需要移动对象的成本，效率也不高（要标记所有存活对象，还要整理所有存活对象的引用地址）
