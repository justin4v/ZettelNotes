...
#### 特点
- 如果多数对象存活的，会产生大量的内存间复制的开销；
- 如果多数对象可回收：
	- 复制开销较低，且是针对半区进行回收；
	- 分配内存*不用考虑空间碎片*，只要移动堆顶指针，按顺序分配即可。
	- 简单，运行高效
- 代价是*可用内存缩小了一半*。
- 现在的商用 Java 虚拟机*大多都优先采用了复制算法去回收新生代*。
- IBM公司曾研究表明——新生代中的对象有 98% 熬不过第一轮收集。因此不需要按照1∶1的比例来划分新生代的内存空间。

#### 改进
- 1989年，Andrew Appel 针对具备“朝生夕灭”特点的对象，提出了更优化的半区复制分代策略，现在称为“Appel式回收”。
...
- HotSpot虚拟机默认Eden和两个Survivor的大小比例是8∶1:1，新生代中可用内存空间为整个新生代容量的90%（Eden的80%加上一个Survivor的10%）；
- 只有一个Survivor空间，10% 的新生代是会被“浪费”。

#### 

98% 的对象可被回收仅仅是“普通场景”下测得的数据，任何人都没有办法百分百保证每次回收都只有不多于10%的对象存活，因此Appel式回收还有一个充当罕见情况的“逃生门”的安
...
- 一开始，对象只会存在于 Eden 区（对象新建时分配空间）和 Survivor from 区（*每次 GC 后交换角色， to 变成了 from*），Survivor  to 是空的。
