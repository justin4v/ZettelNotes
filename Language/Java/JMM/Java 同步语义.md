...
在SW关系中，称同步的起点的活动为释放（release）动作, 终点的活动为获取（acquire）动作。
...
在有了HB关系定义之后，还有另一个**HB一致**的概念：
如果对某一变量的读取操作（r）读到了某个对该变量的写操作（w）的结果，那么 w 既不能在 HB 关系中排在 r 之后 (hb(r, w))， 也不能有另一个对该变量的写操作(w')，在HB关系上介于r与w之间（hb(w, w') 且 hb(w', r)。
