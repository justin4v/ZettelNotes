...
- Java语言中的一些行为具有同步语义，称之为**同步活动（Synchronization Actions)**，提供了对**执行轨迹加以约束的手段**。
...
- CAS 操作：设置锁对象的锁状态；
- 内存屏障将：
	1. 确保加锁前后的数据访问活动不会出现跨屏障的重排；
	2. 同时使缓存失效以确保后续本线程内的内存读都会得到主存中的最新的状态。
- 如果当前的锁对象*被其他线程持有*：
	1. CAS 操作未能成功设置锁状态，加锁操作会将当前线程置于这个锁的等待集上；
	2. 等待当前锁的所有者在后续释放锁，并被唤醒起再继续尝试。
...
- 所以，在语句 2 结束后，一定会观察到 b == true && a == 10 的结果， 无论这两个写是以何种顺序发生。
