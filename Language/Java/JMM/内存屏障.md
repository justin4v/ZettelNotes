...
- 读取时，只有 invalid 状态，需要发送 `Read` ，响应通常很快，不会有冲突。
...
- `CPU 0` 运行过程中出现了 `a` 的两份数据拷贝，`Store Buffer` 和*Cache* 中。
- **写入的更新数据在 `Store Buffer` 中，读取数据却在 *cache*** ，`store buffer` 和 `cache` 数据不同步。
...
- CPU 0 很晚才接收 CPU 1 的响应，使得 **`a=1;` 写操作实际晚于 `b = 1`执行完**；
...
- 本来**可直接写入Cache** 的操作（比如*待操作的数据已经存在于 Cache 中*了）也**必须先存到Store Buffer，然后依序进行刷新**。
