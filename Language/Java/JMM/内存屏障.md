...
1.  CPU 0 执行语句 `a = 1`;
2.  CPU 0 首先从自己的Cache中查找 `a`，发现不存在；
3.  CPU 0 发送 `Read Invalidate` 消息**读取并独占(为了写入)** 含有 `a` 的缓存行，通知其他 CPU，“我要用，你们都销毁！”；
4.  CPU 0 在`Store Buffer`中记录下自己想赋给`a`的值，即 `a = 1`， CPU 0 并不会阻塞，继续向下执行；
5. 此时 CPU 1 可能收到了来自 CPU 0 的 `Read Invalidate` 消息，于是把自己包含 `a` 的缓存行返回给 CPU 0，并且把自己的缓存行状态设置为 `Invalid`；
6.  CPU 0 开始执行 `b = a + 1`；
