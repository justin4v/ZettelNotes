...
- 在如下场景中可能会不符合预期（断言为False）。
	...
	3. 引入 `Store Buffers` 后，*在 `CPU 0` 加载并执行上述代码*（注意：现代计算机中，*线程作为CPU任务调度的基本单位*，**不会被拆分到多个CPU中执行**，所以这里的代码只会被某一个CPU执行），可能的一种顺序如下图所示：
...
8. CPU 0 **从缓存中读取** `a`，此时值为0；
10.  CPU 0 根据自己之前在`Store Buffer`中存放的 `a = 1`来 更新自己Cache中的`a`，设置为1；
11.  CPU 0 在第8步获取的`a`值的基础上`+ 1`（此时不需要重新从缓存行中读取数据，因为读取的动作在第8步中已经做了），并更新自己缓存行中的`b`；此时包含`b`的缓存行的状态为 `Modified`；
12.  CPU 0 执行断言操作，发现断言为 False。
